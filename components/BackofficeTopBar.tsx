...
import {
  Bell, ChevronDown, CircleHelp, Clock3, Command, Download, Home, LogOut,
  Menu, Moon, Plus, RefreshCcw, Search, Settings, Sun, User, Users, Palette,
  ShieldCheck, Box, Building2, KeyRound, Laptop, Mail
} from "lucide-react";

/** Professional Top Navigation for Retail Backoffice (React + Tailwind) */

const BRAND = {
  name: "Retail Backoffice",
  logo: (
    <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 shadow-sm grid place-items-center text-white font-bold">
      R
    </div>
  ),
};
...
function useTheme() {
  const [theme, setTheme] = useState<string>(() =>
    typeof window === "undefined" ? "system" : localStorage.getItem("rb_theme") || "system"
  );
  useEffect(() => {
...
function CommandPalette({
  ...
  const filtered = useMemo(() => {
    ...
    if (!x) return actions;
    return actions.filter((a) => a.label.toLowerCase().includes(x));
  }, [q, actions]);
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
          className="fixed inset-0 z-[60] bg-black/40" onClick={() => setOpen(false)}>
          <motion.div
            initial={{ y: -24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}
          ...
          >
            <div className="flex items-center gap-2 rounded-xl bg-zinc-100 px-3 py-2 dark:bg-zinc-800">
              ...
              <input ref={inputRef} value={q} onChange={(e)=>setQ(e.target.value)}
                placeholder="Search actions, pages, settings…" className="flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-500" />
            ...
            <div className="mt-2 max-h-80 overflow-auto">
              {filtered.length===0 ? (
                <div className="p-4 text-sm text-zinc-500">No matches. Try a different keyword.</div>
              ) : (
                <ul className="divide-y divide-zinc-100 text-sm dark:divide-zinc-800">
                  {filtered.map((a) => (
                    <li key={a.id}>
                      <button onClick={()=>{a.run(); setOpen(false);}}
...
const StatusPill = ({ label = "Live", tone = "success" as "success" | "warning" | "danger" | "info" }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${tones[tone]}`}>
      <span className="h-1.5 w-1.5 rounded-full bg-current"></span>{label}
    </span>
  );
};
...
const Avatar = ({ name = "Ajithan", size = 28 }: { name?: string; size?: number }) => {
  ...
  return (
    <div className="grid place-items-center rounded-full bg-gradient-to-br from-fuchsia-500 to-pink-500 text-white shadow"
...
export default function BackofficeTopBar({
  onNewUser, onImportCSV, onOpenSettings,
  ...
  const refresh = async () => { setBusy(true); await new Promise(r=>setTimeout(r,1200)); setBusy(false); };
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="hidden sm:block">
            ...
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span className="text-zinc-500 dark:text-zinc-400">Directory</span>
            </div>
          </div>
        ...
        <div className="hidden md:flex flex-1 items-center">
          <div className="relative mx-auto w-full max-w-xl">
            ...
            <input
              ...
              placeholder="Search users, roles, stores, permissions… (⌘K)"
              onFocus={() => setCmdOpen(true)} readOnly />
          </div>
        </div>

        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          <div className="hidden xl:block"><StatusPill label="EU-West · Synced" tone="success" /></div>

          <div className="relative">
            <button
              onClick={() => { document.getElementById("rb-quick-create")?.classList.toggle("hidden"); }}
              className="inline-flex items-center gap-1 rounded-xl border border-zinc-200 px-2.5 py-2 text-sm font-medium hover:bg-zinc-50 active:scale-[.99] dark:border-zinc-800 dark:hover:bg-zinc-800/70"
              title="Quick create">
          ...
          <button onClick={refresh} className="rounded-xl p-2 hover:bg-zinc-50 active:scale-[.98] dark:hover:bg-zinc-800/70" title="Refresh data">
          ...
          <div className="relative">
            <button onClick={() => setHelpOpen(v=>!v)} className="rounded-xl p-2 hover:bg-zinc-50 active:scale-[.98] dark:hover:bg-zinc-800/70" title="Help & resources">
          ...
          <div className="relative">
            ...
            <div id="rb-theme-menu" className="hidden absolute right-0 mt-2 w-44 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
              {[{key:"light",icon:<Sun className="h-4 w-4" />,label:"Light"},
                ...
                {key:"system",icon:<Laptop className="h-4 w-4" />,label:"System"},].map(m => (
                ...
                <motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}}
                  ...
                  <div className="divide-y divide-zinc-100 text-sm dark:divide-zinc-800">
                    <div className="py-1">
                      ...
                      <button className="flex w-full items-center gap-2 px-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-800" onClick={() => onOpenSettings?.()}><Settings className="h-4 w-4" /> Settings</button>
...
export function DemoPage() {
  return (
    <div className="min-h-[120vh] bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      <BackofficeTopBar
        onNewUser={() => alert("Create user modal…")}
        onImportCSV={() => alert("Import CSV flow…")}
        onOpenSettings={() => alert("Open settings…")}
      />
      <main className="mx-auto max-w-screen-2xl px-4 py-8">
        <h1 className="text-2xl font-semibold">Users</h1>
        <p className="mt-2 text-zinc-600 dark:text-zinc-400">
          Example content area. Scroll to see sticky top bar and interact with the right-side menus.
        </p>
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 18 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <div className="mb-2 text-sm font-medium">User card {i + 1}</div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400">Some placeholder content to simulate a grid.</div>
            </div>
          ))}
        </div>
      </main>
